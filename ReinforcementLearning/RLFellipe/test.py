...
class GridGenerator(QWidget):
    ...
    def __init__(self, main_window: QMainWindow):
        ...
        goal_controls_layout.addStretch()
        # Removed tight_layout to manage manually
        ...
        # Use consistent colors: Green for Start, Red for Goal
        colors = ["#ffffff", "#555555", "#2ecc71",
                  "#e74c3c"]  # Free, Obstacle, Start, Goal
    ...
    def update_grid(self, grid_size: Tuple[int, int], start_pos: Tuple[int, int, int], goal_pos: Tuple[int, int, int]):
        ...
        self.display_map[self.start_pos[1],
                         self.start_pos[0]] = 2  # 2 for Start
        self.display_map[self.goal_pos[1], self.goal_pos[0]] = 3  # 3 for Goal
    ...
    def setup_plot(self):
        ...
        # --- MODIFIED: Add titles, labels, and legend ---
        self.ax.set_title(
...
class InteractiveResultsPage(QWidget):
    ...
    def setup_page(self):
        ...
        if not env:
            return
        # Default to env start, converting from (row, col) to (r, c)
        self.selected_start_pos = (env.start[0], env.start[1])
        # Convert angle index back to 0-7 range for compass
        start_angle_rad = env.start[2]
        start_angle_idx = env.angle_to_idx(start_angle_rad)
        self.start_compass.set_orientation(start_angle_idx)

        self.instruction_label.setText(
            f"Start point selected at (Y={env.start[0]}, X={env.start[1]}).\nClick grid to change or visualize path.")
        self.draw_base_grid()
        self.draw_path_from_point(self.selected_start_pos, start_angle_idx)
    ...
    def draw_base_grid(self):
        ...
        self.ax.legend(bbox_to_anchor=(1.04, 1),
    ...
    def draw_path_from_point(self, start_pos, start_orientation_idx):
        env = self.main_window.trained_env
        ...
        start_k = start_orientation_idx

        self.draw_base_grid()  # This already sets up the legend box and axis titles

        eps_bak = agent.epsilon
        agent.epsilon = 0.0

        start_angle_rad = env.idx_to_angle(start_k)
        env.reset_new_position((start_r, start_c, start_angle_rad))
        s = env.agent_pos
        path = [s]
        ...
        xs, ys, us, vs = [], [], [], []
        arrow_scale = 0.35
        for (pr, pc, p_ang_rad) in path:
            xs.append(pc)
            ys.append(pr)
            us.append(math.cos(p_ang_rad) * arrow_scale)
            vs.append(math.sin(p_ang_rad) * arrow_scale)
        if xs:
        ...
        # --- MODIFIED: Update legend and layout ---
        self.ax.legend(bbox_to_anchor=(1.04, 1),
                       loc='upper left', borderaxespad=0.)
        self.fig.tight_layout(rect=[0, 0, 0.85, 1])
        self.canvas.draw()
